...
export default async function ReportMonitoringPage({
  ...
  return (
    <>
      <Topbar title="Report Monitoring" breadcrumb="Audit" breadcrumbHref="/audit" />
      <div className="app-content">
        <ReportMonitoringView
          ...
          services={rows}
          canWrite={canWrite}
          prevHref={`/audit/report-monitoring?year=${prevYear}`}
          nextHref={`/audit/report-monitoring?year=${nextYear}`}
          thisYearHref={`/audit/report-monitoring?year=${new Date().getFullYear()}`}
        />
      </div>
    </>
  );
